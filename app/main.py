...
import math
import os
from typing import Any, Awaitable, Callable, Coroutine, Generic, Optional, Tuple, TypeVar, Union, Type
...
Z = TypeVar("Z")
def get_dict_value(data: Any, key: str, t: type[T], d: Z = None) -> T | Z:
    if data is None:
        return d
...
class Listeners(Generic[S]):
    _listeners: dict[Type[S], set[Callable[[S], Coroutine[Any, Any, None]]]]

    def __init__(self):
        self._listeners = {}

    def add(self, *cbs: Callable[[S], Coroutine[Any, Any, None]]) -> None:
        for cb in cbs:
            sig = inspect.signature(cb)
            first_arg = list(sig.parameters.keys())[0]
            key = sig.parameters[first_arg].annotation
    ...
    async def notify(self, key: type, message: S) -> None:
        for cb in self._listeners.get(key, set()):
            await cb(message)


class MagnetLinkTracker(object):
    magnet_link: dict[str, ValueType]
    tracker_data: dict[str, ValueType]
    handshake: Optional[TrackerPeersResponse] = None

    _tracker: Optional[Tracker]
    _peers: list[Peer]
    _listeners: Listeners

    def __init__(self, magnet_link: str) -> None:
        self.magnet_link = parse_magnetic_link(magnet_link)
        self.tracker_data = query_tracker(self.magnet_link)
        self._peers = get_peers(self.tracker_data)
        self._listeners = Listeners()
        self._tracker = None

    async def __aenter__(self) -> "MagnetLinkTracker":
        if len(self._peers) == 0:
            raise ValueError("No peers found")

        peer = self._peers[0]
        self._tracker = Tracker((peer.ip, peer.port))
        await self._tracker.__aenter__()

        self.handshake = await tracker_handshake(
            self.magnet_link,
            self._tracker,
    ...
    async def communicate(self) -> None:
        ...
        message = await self._tracker.listen_for_messages()

        if message is not None:
            await self._listeners.notify(type(message), message)

    async def send_message(self, message: PeerMessage) -> None:
        if self._tracker is None:
            raise ValueError("Tracker not initialized")

        await self._tracker.send_message(message)
...
async def main():
    ...
    magnet_download_piece_cmd = subparsers.add_parser("magnet_download_piece")
    magnet_download_piece_cmd.add_argument("-o", help="file to save the piece to", default="-")
    ...
    match args.command:
        ...
        case "magnet_handshake":
            ...
            async with MagnetLinkTracker(args.magnet_link) as tracker:
                if tracker.handshake is not None:
                    print("Peer ID:", tracker.handshake.peer_id.hex())
                ...
                tracker.listeners().add(bitfield, extension)
        ...
        case "magnet_info":
            ...
            async with MagnetLinkTracker(args.magnet_link) as tracker:
                print(f"Tracker URL: {get_torrent_url(tracker.magnet_link)}")
        ...
        case "magnet_download_piece":
            ...
            async with MagnetLinkTracker(args.magnet_link) as tracker:
                ...
                async def unchoke(_: Unchoke) -> None:
                    nonlocal piece_index, piece_length, hashes, length, chunks
                    if tracker._tracker is not None:
                        print("Starting download", piece_index, piece_length)
                        # TODO: refactor me
                    ...
                    piece = PieceMeta(
                        ...
                        length=last_piece_size if piece_index == last_piece_index else piece_length,
                        pipeline_size=16 * 1024,
                    )

                    chunks = piece.pipeline_chunks()
                ...
                async def piece(piece: Piece) -> None:
                    nonlocal chunks, listening, piece_length
 
                    chunks = [
                        chunk
                        for chunk in chunks
                        if chunk.begin != piece.begin
                    ]

                    seek_offset = -abs(piece_index * piece_length)
                    with open(output_file, "r+b") as f:
                        print("Writing chunk", "begin:", piece.begin, "seek:", piece_index * piece_length + piece.begin + seek_offset)
                        f.seek(piece_index * piece_length + piece.begin + seek_offset, os.SEEK_SET)
                        f.write(piece.block)
                        f.flush()

                    print("Downloaded chunk", piece.index, piece.begin, len(piece.block))
